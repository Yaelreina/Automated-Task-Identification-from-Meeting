...
# Configuration
# Path updated to relative for submission portability
MODEL_PATH = "../Code/outputs"
DATA_DIR = "../Data" 
TEST_FILE = os.path.join(DATA_DIR, "test.jsonl")
REPORT_FILE = "final_stable_report.txt"
CSV_FILE = "stable_analysis_results.csv"
...
# Robust Extraction
def extract_robust_json(text, sample_id):
    # Clean everything that isn't the JSON itself
    text = text.replace("```json", "").replace("```", "").strip()

    # 1. Try to find the start of a list or object
    start_brace = text.find('{')
    start_bracket = text.find('[')

    if start_brace == -1 and start_bracket == -1:
        print(f"\n[Sample {sample_id}] No JSON Start Found.")
    ...
    try:
        ...
        try:
            ...
            # Step 2: AST (Python literal eval)
            try:
                return ast.literal_eval(candidate)
            except:
                # Step 3: Raw JSONDecoder
                decoder = json.JSONDecoder()
                obj, _ = decoder.raw_decode(candidate)
                return obj
    except Exception as e:
        print(f"\n[Sample {sample_id}] Parse Fail: {e}")
        return None


# Original Prompt (Same as Training)
alpaca_prompt = """Below is an instruction that describes a task, paired with an input that provides further context. Write a response that appropriately completes the request.

### Instruction:
Extract action items from the conversation into a JSON format with 'assignee', 'action', and 'deadline'.
...
# The starting string we force inject (Schema Forcing)
FORCED_START = '{"action_items": ['

print("--- Starting STABLE Evaluation (Zero-Shot + Forced Start) ---")
...
for i, row in tqdm(enumerate(dataset), total=len(dataset)):
    # 1. Build prompt + forced start
    prompt = alpaca_prompt.format(row['input']) + FORCED_START
    ...
    # 3. Extract only the model's new output
    # Splitting by FORCED_START to get the generated suffix
    raw_suffix = decoded.split(FORCED_START)[-1]
    # Clean end tokens
    raw_suffix = raw_suffix.split("<|end_of_text|>")[0]

    # Reassemble to get full JSON
    full_text = FORCED_START + raw_suffix

    parsed_json = extract_robust_json(full_text, i + 1)
    ...
    try:
        ...
        if isinstance(parsed_json, dict):
            items = parsed_json.get("action_items", [])
        ...
        s_assignee, s_action, s_deadline = calculate_complex_metrics(items, true_json.get('action_items', []))
        ...
        detailed_results.append({
            ...
            "Prediction": json.dumps(items, ensure_ascii=False),
            "Ground Truth": json.dumps(true_json.get('action_items'), ensure_ascii=False)
        })

        # Short print for success
        print(f"Sample {i + 1}: OK")

    except Exception as e:
        print(f"Logic Error {i}: {e}")
        failures += 1
